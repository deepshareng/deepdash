...
var renderApps = function(apps, appIndex, callback) {
    var tplList = [];
    $('#app-name').text(apps[appIndex].appname);
    for (var i in apps) {
    ...
    $('#slidebar-app-name').html(apps[appIndex].appname);
...
$(function() {
    ...
    $('#submit-callback').on('click', function() {
        ...
        if (callbackUrl.length <= 0) {
            $('#callback-url-group').addClass('has-error');
            return;
        }

        DSAPI.saveCallbackUrl(sessionAppId, {'attributionpushurl': callbackUrl}, function() {
            DSCOMMON.showModal('回调测试', '保存回调URL成功');
        }, function() {
            DSCOMMON.showModal('回调测试', '保存回调URL失败');
        });
    });

    $('#test-callback').on('click', function() {
        var testData = $('#callback-test-data').val();
        var callbackUrl = $('#callback-url').val();

        DSAPI.testCallback(sessionAppId, {data: testData, url: callbackUrl}, function() {
            // alert success 
            DSCOMMON.showModal('回调测试', '测试成功');
        }, function(xhr, s, e) {
            // alert fail 
            DSCOMMON.showModal('回调测试', '测试失败');
        });
    });

    DSAPI.getAppInfo(sessionAppId, function(data) {
        $('#callback-url').val(data.attributionpushurl);
    }, function() {
        DSCOMMON.showModal('回调测试', '获取不了App信息<br/>请刷新页面');
    });
});

},{"../lib/api.js":1,"../lib/common.js":2}]},{},[4])