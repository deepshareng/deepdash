...
func AddHandler(mux *http.ServeMux, endpoint string, dbHour, dbDay, dbTotal *mgo.Database, hourCollNamePrefix, dayCollNamePrefix, totalCollNamePrefix string) {
	mux.Handle(
		endpoint,
		newCounterHandler(
			NewHourAggregateService(dbHour, hourCollNamePrefix),
			NewDayAggregateService(dbDay, dayCollNamePrefix),
			NewTotalAggregateService(dbTotal, totalCollNamePrefix),
		),
	)
}
...
func (ch *counterHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	startTime := time.Now()
	defer in.PromCounter.HTTPGETDuration(time.Since(startTime))
	...
	log.Infof("Request Detail, AppID=%s, Channel=%s, Event=%v, Start=%s, Granularity=%s, Limit=%d", appid, fields[0], eventFilters, start.String(), gran.String(), limit)
	aggrs, err := ags.QueryDuration(appid, fields[0], eventFilters, start, gran, limit)
	if err != nil {
