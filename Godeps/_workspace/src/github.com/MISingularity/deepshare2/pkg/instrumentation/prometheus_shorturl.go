...
func (ps *prometheusShorturl) HTTPGetDuration(d time.Duration) {
	ps.httpGetDuration.Observe(float64(d) / float64(time.Millisecond))
}
func (ps *prometheusShorturl) HTTPPostDuration(d time.Duration) {
	ps.httpPostDuration.Observe(float64(d) / float64(time.Millisecond))
}

func (ps *prometheusShorturl) StorageGetDuration(d time.Duration) {
	ps.storageGetDuration.Observe(float64(d) / float64(time.Millisecond))
}

func (ps *prometheusShorturl) StorageSaveDuration(d time.Duration) {
